...
elif [ "${OS_ARCH,,}" == "aarch64" ] && [ $OS_VERSION == 1804 ]; then
	echo "Installing on rock64 - ubuntu 18.04"
...
function install_ubuntu_dependencies
{
	...
	exit_on_error "$SUDO_PREFIX apt-get $APT_QUIET install -y libopencv-dev"
...
function install_rock64_dependencies
{
	exit_on_error "$SUDO_PREFIX apt-get $APT_QUIET update"
	exit_on_error "$SUDO_PREFIX apt-get $APT_QUIET install -y unzip coreutils curl git python3 python3-pip"
	exit_on_error "$SUDO_PREFIX apt $APT_QUIET install -y python3-setuptools"
	exit_on_error "$SUDO_PREFIX apt $APT_QUIET install -y python-setuptools"
	exit_on_error "$SUDO_PREFIX apt-get $APT_QUIET install -y libusb-1.0-0-dev"
	exit_on_error "$SUDO_PREFIX apt $APT_QUIET install -y aptitude"
	sudo aptitude install libboost-all-dev
	exit_on_error "$SUDO_PREFIX apt-get $APT_QUIET install -y $(cat "$DIR/requirements_apt.txt")"
	exit_on_error "$SUDO_PREFIX apt-get $APT_QUIET install -y --no-install-recommends"
	exit_on_error "$SUDO_PREFIX apt-get $APT_QUIET install -y libopencv-dev"
	$SUDO_PREFIX ldconfig
}
...
function install_rock64_python_dependencies
{
        echo "Installing python dependencies..."
	...
	exit_on_error "$PIP_PREFIX pip3 install $PIP_QUIET graphviz"
	# exit_on_error "$PIP_PREFIX pip3 install $PIP_QUIET --upgrade numpy==1.13.1"
	exit_on_error "$SUDO_PREFIX apt $APT_QUIET install -y python3-h5py"
	exit_on_error "$SUDO_PREFIX apt $APT_QUIET install -y python3-lxml"
